...
const translations = {
  en: {
    ...
    noticeImageLocationDiagnostics:
      "Geo Capture diagnostics: image={image}; local={local}; exif={exif}; exifError={exifError}; metadata={metadata}; entries={entries}; gpsEntries={gpsEntries}; match={match}; reason={reason}",
  ...
  "zh-TW": {
    ...
    noticeImageLocationDiagnostics:
      "Geo Capture 診斷：圖片={image}；本機={local}；EXIF={exif}；EXIF錯誤={exifError}；metadata={metadata}；條目={entries}；GPS條目={gpsEntries}；匹配={match}；原因={reason}",
  ...
  "zh-CN": {
    ...
    noticeImageLocationDiagnostics:
      "Geo Capture 诊断：图片={image}；本地={local}；EXIF={exif}；EXIF错误={exifError}；metadata={metadata}；条目={entries}；GPS条目={gpsEntries}；匹配={match}；原因={reason}",
